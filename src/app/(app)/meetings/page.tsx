        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <div className={cn("w-12 h-12 rounded-lg flex flex-col items-center justify-center text-white font-bold", meeting.status === "scheduled" ? "bg-brand-teal" : meeting.status === "completed" ? "bg-green-600" : "bg-red-600")}>
                <span className="text-xs leading-none">{new Date(meeting.meeting_date).toLocaleDateString("en", { month: "short" })}</span>
                <span className="text-lg leading-none">{new Date(meeting.meeting_date).getDate()}</span>
              </div>
              <div>
                <h3 className="font-semibold text-text-primary">{meeting.title}</h3>
                <p className="text-sm text-text-secondary">{meeting.companies?.company_name || "Unknown"}</p>
              </div>
              <Badge className={cn("ml-2", STATUS_COLORS[meeting.status])} variant="outline">
                {meeting.status.charAt(0).toUpperCase() + meeting.status.slice(1).replace("_", " ")}
              </Badge>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              {meeting.contacts?.full_name && <div className="flex items-center gap-2 text-text-secondary"><Users className="h-4 w-4 text-text-muted" /><span>{meeting.contacts.full_name}</span></div>}
              <div className="flex items-center gap-2 text-text-secondary"><Clock className="h-4 w-4 text-text-muted" /><span>{new Date(meeting.meeting_date).toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit" })} ({meeting.duration_minutes} min)</span></div>
              {meeting.location && <div className="flex items-center gap-2 text-text-secondary col-span-2"><MapPin className="h-4 w-4 text-text-muted" /><span className="truncate">{meeting.location}</span></div>}
            </div>
            {meeting.description && <p className="text-sm text-text-muted mt-3 italic">{meeting.description}</p>}
          </div>
          <div className="flex items-center gap-2">
            {meeting.status === "scheduled" && (
              <>
                <Button size="sm" variant="outline" onClick={() => handleStatusUpdate(meeting.id, "completed")}><CheckCircle2 className="h-3 w-3 mr-1" /> Done</Button>
                <Button size="sm" variant="outline" onClick={() => handleStatusUpdate(meeting.id, "cancelled")}><XCircle className="h-3 w-3 mr-1" /> Cancel</Button>
              </>
            )}
            <Button size="sm" variant="ghost" className="text-red-500 hover:text-red-700" onClick={() => handleDelete(meeting.id)}><Trash2 className="h-3 w-3" /></Button>
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500">Schedule and track your meetings</p>
        </div>
        <Button className="bg-brand-teal hover:bg-brand-teal-dark text-white" onClick={() => setBookDialogOpen(true)}>
