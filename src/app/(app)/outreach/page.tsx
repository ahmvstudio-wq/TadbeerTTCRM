...
import {
  ...
  Eye, Edit, Trash2, ArrowRight, Bell, Search, Rocket,
} from "lucide-react";
...
interface TouchRecord { id: string; lead_id: string; channel: string; step: number; message: string; status: string; sent_at: string; response?: string; follow_up_date?: string; campaign_id?: string; }
...
export default function OutreachPage() {
  ...
  // ─── GATE ─────────────────────────────────────────────────────────
  if (phase === "gate") {
    const touchedIds = new Set(touches.map((t) => t.lead_id));
    const untouchedList = allLeads.filter((l) => !touchedIds.has(l.id));
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <Card className="hover-lift press-effect cursor-pointer border-2 border-dashed border-brand-teal/40 hover:border-brand-teal transition-all" onClick={async () => { await loadAllLeads(); setNewCampaignOpen(true); }}>
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">Select untouched prospects and start a new outreach campaign</p>
              </div>
            </CardContent>
          </Card>
                    ...
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-text-muted">{camp.lead_ids.length} prospects · {campTouches.length} touches · {new Date(camp.created_at).toLocaleDateString()}</p>
                        </div>
                      </div>
          ...
          <Card className="hover-lift press-effect cursor-pointer border-2 border-transparent hover:border-brand-gold transition-all" onClick={async () => { await loadAllLeads(); setSelectedLeads(allLeads); setPhase("active"); }}>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-text-primary">Quick Start — All Prospects</h3>
                <p className="text-sm text-text-secondary">Jump straight in with all {allLeads.length} prospects</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Dialog open={newCampaignOpen} onClose={() => setNewCampaignOpen(false)} className="max-w-3xl">
            ...
            <DialogContent className="space-y-4">
              ...
              <div>
                ...
                <div className="max-h-[300px] overflow-y-auto border border-border rounded-xl divide-y divide-border-light">
                  {untouchedList.length === 0 ? <div className="p-6 text-center text-sm text-text-muted">All prospects have been touched.</div> : untouchedList.map((lead) => (
            ...
            <DialogFooter>
              ...
              <Button onClick={() => {
                ...
                addToast("success", `Campaign "${c.name}" created with ${c.lead_ids.length} prospects`);
    ...
    <div className="space-y-5 page-enter">
      ...
      {pendingFollowUps.length > 0 && (
        <Card className="hover-lift border-amber-200 bg-amber-50/30"><CardContent className="p-3">
          ...
          <div className="flex flex-wrap gap-1.5">{pendingFollowUps.slice(0, 5).map((t) => { const l = selectedLeads.find((x) => x.id === t.lead_id); return <button key={t.id} onClick={() => setExpandedLead(t.lead_id)} className="px-2 py-1 rounded-full bg-white border border-amber-200 text-[10px] text-amber-700 hover:bg-amber-100 transition-colors press-effect">{l?.contact_name || l?.company_name} · {t.channel}</button>; })}</div>
        </CardContent></Card>
      )}
      ...
      <div className="space-y-2 stagger-children">
        {filteredLeads.map((lead) => {
          ...
          const lastTouch = leadTouches[0]; const hasFollowUp = pendingFollowUps.some((t) => t.lead_id === lead.id);
          return (
            <Card key={lead.id} className={cn("hover-lift transition-all", hasFollowUp && "border-amber-300")}>
              <CardContent className="p-0">
                <div className="flex items-center gap-3 py-3 px-4 cursor-pointer hover:bg-cream-dark/30 transition-colors" onClick={() => setExpandedLead(isExpanded ? null : lead.id)}>
                  ...
                  <div className="h-9 w-9 rounded-lg bg-brand-teal-light flex items-center justify-center flex-shrink-0"><Users className="h-4 w-4 text-brand-teal" /></div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2"><span className="text-sm font-semibold text-text-primary">{lead.contact_name || lead.company_name}</span>{lead.contact_name && lead.company_name && <span className="text-xs text-text-muted">· {lead.company_name}</span>}</div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    {CHANNELS.map((ch) => { const chT = leadTouches.filter((t) => t.channel === ch.id); const Icon = ch.icon; return (
                      <button key={ch.id} onClick={() => openTouchDialog(lead.id, ch.id)} className={cn("h-8 w-8 rounded-lg flex items-center justify-center transition-all press-effect", chT.length > 0 ? ch.color + " ring-2 " + ch.ring : "bg-cream-dark text-text-muted hover:bg-white border border-border")} title={`${ch.label} (${chT.length})`}><Icon className="h-3.5 w-3.5" /></button>
                    ); })}
                  </div>
                </div>
                {isExpanded && (
                  <div className="border-t border-border-light px-4 py-3 animate-expand-down">
                    ...
                    ) : <p className="text-xs text-text-muted text-center py-2">No touches yet. Use channel buttons above.</p>}
                  </div>
                )}
              </CardContent>
            </Card>
