...
export async function updateCompany(id: string, data: { company_name?: string; industry?: string; website?: string; phone?: string; email?: string; country?: string; city?: string; employee_count?: number; notes?: string }) {
  try {
    const { data: company, error } = await supabase.from('companies').update({ ...data, updated_at: new Date().toISOString() }).eq('id', id).select().single()
    if (error) return { data: null, error: error.message }
    return { data: company, error: null }
  } catch (error) {
    return { data: null, error: error instanceof Error ? error.message : 'An unexpected error occurred' }
...
export async function updateCompanyStatus(id: string, status: string) {
  try {
    const { data: company, error: updateError } = await supabase.from('companies').update({ status, updated_at: new Date().toISOString() }).eq('id', id).select().single()
    if (updateError) return { data: null, error: updateError.message }
    await supabase.from('activities').insert({ company_id: id, activity_type: 'status_changed', title: 'Company status updated', description: `Status changed to "${status}"`, metadata: { new_status: status }, created_at: new Date().toISOString() })
    return { data: company, error: null }
  } catch (error) {
    return { data: null, error: error instanceof Error ? error.message : 'An unexpected error occurred' }
  }
